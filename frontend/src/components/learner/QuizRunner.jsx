...
export default function QuizRunner({ lesson, enrollmentId, onComplete }) {
    ...
    const [proctorData, setProctorData] = useState({ tabSwitches: 0, fullScreenExits: 0, violations: [], webcamEnabled: false });
    ...
    const handleFinish = async (finalScore) => {
        ...
        // Save attempt to database for instructor tracking
        try {
            const enrollment = await axios.get(`${API_URL}/enrollments/my-courses`);
            const currentEnrollment = enrollment.data.find(e => e._id === enrollmentId);

            if (currentEnrollment) {
                await axios.post(`${API_URL}/attempts`, {
                    userId: currentEnrollment.userId,
                    courseId: currentEnrollment.courseId._id || currentEnrollment.courseId,
                    quizId: quiz._id,
                    score: scorePercent,
                    total: 100,
                    passed: passed,
                    answers: {},
                    proctoring: proctorData // Include proctoring data
                });
            }
        } catch (err) {
    ...
    const handleProctorViolation = (violation) => {
        if (violation.getData) {
            // Store the getData function for later use
            return;
        }

        setProctorData(prev => ({
            ...prev,
            tabSwitches: violation.type === 'tab_switch' ? violation.count : prev.tabSwitches,
            fullScreenExits: violation.type === 'fullscreen_exit' ? violation.count : prev.fullScreenExits,
            violations: [...prev.violations, { type: violation.type, timestamp: violation.timestamp }]
        }));
    };
    ...
    // Render ProctorMonitor and quiz content
    return (
        <>
            {/* Only activate proctoring if enabled for this quiz */}
            <ProctorMonitor
                isActive={gameState === 'playing' && quiz.proctoringEnabled}
            ...
            return (
                <div className="p-8 md:p-12 text-center max-w-2xl mx-auto animate-in fade-in zoom-in duration-300">
                    ...
                    {quiz.proctoringEnabled && (
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 rounded-full border border-amber-200 mb-6 mx-auto">
                            <AlertCircle size={16} />
                            <span className="text-xs font-bold uppercase tracking-wider">Proctored Quiz</span>
                        </div>
                    )}
                        ...
                        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 mb-8 text-left">
                            ...
                            <ul className="space-y-2 text-sm text-slate-600">
                                <li className="flex items-start gap-2">
                                    ...
                                    <span>Full-screen mode is required throughout the quiz.</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="text-slate-400">•</span>
                                ...
                                <li className="flex items-start gap-2">
                                    <span className="text-slate-400">•</span>
                                    <span>Copying and pasting is disabled.</span>
                                </li>
                            </ul>
                        </div>
                    )}
