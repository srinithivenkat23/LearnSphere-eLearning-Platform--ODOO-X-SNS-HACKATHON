    ...
    const handleSaveLesson = async (lessonData) => {
        try {
            if (editingLesson) {
                ...
                // Local sync
                onChange('lessonsCount', (course.lessonsCount || 0) + 1);
            }
            loadLessons();
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this content? This action cannot be undone.")) {
            ...
            try {
                ...
                await updateDoc(doc(db, 'courses', course.id), {
                    lessonsCount: Math.max(0, (course.lessonsCount || 0) - 1)
                });
                onChange('lessonsCount', Math.max(0, (course.lessonsCount || 0) - 1));
                loadLessons();
            } catch (err) {
                console.error(err);
            }
                ...
                {lessons.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {lessons.map((lesson, index) => (
                            <div key={lesson.id} className="group flex items-center gap-4 bg-white border border-slate-100 p-4 rounded-2xl hover:border-blue-200 hover:shadow-xl hover:shadow-blue-500/5 transition-all">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className={`p-2 rounded-xl transition-all ${activeMenu === lesson.id ? 'bg-slate-900 text-white' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-50'}`}
                                    >
