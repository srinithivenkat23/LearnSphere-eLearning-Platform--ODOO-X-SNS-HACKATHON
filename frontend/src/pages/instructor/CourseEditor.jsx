...
export default function CourseEditor() {
    ...
    const handleSave = async () => {
        ...
        if (course.published && !course.websiteUrl?.trim()) {
            alert("Website URL is required when course is published");
        ...
        try {
            ...
            // Show a temporary success state or just keep working
    ...
    const togglePublish = async () => {
        ...
        if (newState && !course.websiteUrl?.trim()) {
            alert("Please provide a website URL in the Description tab before publishing.");
            setActiveTab('description');
            return;
        }

        setCourse(prev => ({ ...prev, published: newState }));
        // Autosave the published state
        try {
            const courseRef = doc(db, 'courses', courseId);
            await updateDoc(courseRef, { published: newState, updatedAt: serverTimestamp() });
        } catch (err) {
            console.error(err);
        }
    };
        ...
        <div className="flex flex-col h-screen bg-slate-50 overflow-hidden">
            ...
            <div className="bg-white border-b border-slate-200 px-8 py-6 z-20">
                <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="shrink-0">
                        <div className="relative group w-full lg:w-48 h-32 rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden flex flex-col items-center justify-center transition-all hover:bg-slate-100 hover:border-blue-400 cursor-pointer">
                            {course.imageUrl ? (
                                <>
                            ...
                            <input
                                ...
                                className="absolute bottom-2 left-2 right-2 px-2 py-1 text-[10px] bg-white border border-slate-200 rounded opacity-0 group-hover:opacity-100 transition-opacity outline-none focus:ring-1 focus:ring-blue-500"
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <select
                                    ...
                                    value={course.responsibleId || userData?.uid}
                                    onChange={(e) => {
                                        const selected = e.target.value;
                                        handleChange('responsibleId', selected);
                                        // Mock name assignment
                                        handleChange('responsibleName', selected === userData?.uid ? userData?.name : 'Assistant Admin');
                                    }}
                                >
                                    <option value={userData?.uid}>{userData?.name} (Me)</option>
                                    <option value="admin_assistant">Assistant Admin</option>
                                </select>
                            </div>
            ...
            <AddAttendeeModal
                ...
                onAdd={(data) => {
                    ...
                    alert(`Learner ${data.name} invited!`);
                }}
            />
