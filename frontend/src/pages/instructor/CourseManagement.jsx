...
export default function InstructorCourses() {
    ...
    const fetchCourses = async () => {
        ...
        try {
            const params = {
                role: userData?.role,
                all: true // To get both drafts and published
            };

            // Only filter by instructorId if the user is an instructor (not admin)
    ...
    const handleCreateCourse = async (e) => {
        ...
        try {
            const response = await axios.post(`${API_URL}/courses`, {
                ...
                instructorName: userData?.name || user.email,
                ...
                price: 0,
                accessRule: 'open',
                description: 'Write your course description here...',
    ...
    const handleTogglePublish = async (course) => {
        ...
        try {
            ...
            setCourses(courses.map(c => (c._id || c.id) === id ? { ...c, published: newStatus } : c));
        } catch (err) {
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">
                            {userData?.role === 'admin' ? 'Global Courses Catalog' : 'Instructor Dashboard'}
                        </h1>
                        <p className="text-slate-500 mt-1">
                            {userData?.role === 'admin'
                                ? 'Monitor and manage all courses across the entire LearnSphere platform.'
                                : 'Manage, track performance, and create new learning experiences.'}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredCourses.map(course => (
                            <CourseKanbanCard
                                key={course._id}
                                course={course}
                                onEdit={() => navigate(`/instructor/courses/${course._id}`)}
        ...
        <Card className="group flex flex-col h-full bg-white border border-slate-200 hover:border-blue-400 hover:shadow-2xl hover:shadow-blue-500/5 transition-all duration-300 rounded-3xl overflow-hidden p-0">
            <div className="h-44 bg-slate-100 relative overflow-hidden shrink-0">
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between">
                    ...
                    <button onClick={(e) => { e.preventDefault(); onShare(course._id); }} className="p-2 bg-white/20 backdrop-blur-md rounded-lg text-white hover:bg-white hover:text-slate-900 transition-colors">
                        <Share size={14} />
                    </button>
                </div>
            </div>
        ...
        <tr className="hover:bg-slate-50 transition-colors group">
            ...
            <td className="px-6 py-4 text-right">
                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => onShare(course._id)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-slate-200" title="Share">
                        <Share size={16} />
                    </button>
