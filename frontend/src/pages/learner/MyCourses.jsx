...
export default function MyCourses() {
    ...
    const [certificateData, setCertificateData] = useState({ isOpen: false, courseTitle: '' });
    ...
    const fetchMyCourses = async () => {
        try {
            ...
            enrollData.forEach(item => {
                ...
                if (course) {
                    enrollmentsMap[course._id] = enrollment;
                    coursesData.push({ ...course, id: course._id });
                    totalProgress += enrollment.progressPercent || 0;
                }
            });

            setEnrollments(enrollmentsMap);
            setCourses(coursesData);
            setAvgCompletion(enrollData.length > 0 ? Math.round(totalProgress / enrollData.length) : 0);
        } catch (error) {
    ...
    const handleStartCourse = async (enrollmentId) => {
        try {
            // "Starting" a course sets it to 5% progress and gives points
            await axios.put(`${API_URL}/enrollments/${enrollmentId}/progress`, { progressPercent: 5 });
            fetchMyCourses(); // Refresh dashboard
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-white font-sans">
                        ...
                        {filteredCourses.length === 0 ? (
                            ...
                            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 gap-8" : "space-y-4"}>
                                {filteredCourses.map(course => {
                                                    ...
                                                    <div className="mt-auto space-y-4">
                                                        <div className="space-y-2">
                                                            <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                                                                ...
                                                                <span className="text-slate-400 italic">
                                                                    {enrollment.lastActive ? `Active ${new Date(enrollment.lastActive).toLocaleDateString()}` : 'Not started'}
                                                                </span>
                                                            </div>
                                                        ...
                                                        <div className="flex gap-2">
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    if (!hasStarted && !isCompleted) {
                                                                        handleStartCourse(enrollment.id || enrollment._id);
                                                                    } else {
                                                            ...
                                                            {isCompleted && (
                                                                <Button
                                                                    variant="outline"
                                                                    className="px-4 border-slate-200 text-slate-600 hover:bg-slate-50"
                                                                    onClick={() => setCertificateData({ isOpen: true, courseTitle: course.title })}
                                                                >
                                                                    <Download size={18} />
                                                                </Button>
                                                            )}
                                    ...
                                    return (
                                        <div key={course._id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-6 group hover:border-blue-200 transition-all">
            ...
            <Certificate
                ...
                onClose={() => setCertificateData({ ...certificateData, isOpen: false })}
